...
* 共享的关系型数据库（必须是可跨网络访问的 MySQL 或 PostgreSQL，**千万不要在集群模式下使用 SQLite**）
* 共享的 Redis 服务（用于节点间数据同步、缓存更新和高频额度扣减防护）
* 可选：负载均衡器（如 Nginx、HAProxy 或云服务商提供的负载均衡服务）

## 集群架构概述

New API 集群采用主从架构设计。请注意，这里的“主从”并非传统数据库层面的读写分离，而是为了**切分系统的定时任务处理权**：

1. **主节点 (`master`)**：除了处理普通的用户 HTTP 请求，还全权负责在后台定时执行各种全局异步任务（如：渠道状态监控探测、全局模型更新、批量配额对账、订阅续费扣款等）。
2. **从节点 (`slave`)**：剥离了所有后台定时同步脚本和定时器，退化为纯粹的、无状态的网络并发请求网关。它们只负责光速验证鉴权，并将大模型请求投递给上游，从根源上杜绝多台机器半夜同时跑对账单而导致的「数据库行锁冲突死锁」和「配额多扣」。

<img alt="集群架构" src={__img0} />
...
首先，您需要准备共享的 MySQL 或 PostgreSQL 数据库和 Redis 服务。这可以是：

* 单独部署的高可用 MySQL/PostgreSQL 和 Redis 服务
* 云服务商提供的托管数据库和缓存服务
* 独立的服务器上运行的数据库和 Redis
...
server {
    ...
    location / {
        ...
        # 【极其重要】适应大语言模型 Server-Sent Events (SSE) 流式响应
        proxy_buffering off;
        proxy_cache off;
        
        # 延长超时时间以防止等待大模型深度思考回复时断流
...
* 检查 Redis 连接是否正常
* 确认 SESSION\_SECRET 和 CRYPTO\_SECRET 在所有节点上是否相同
* 验证数据库连接配置是否正确
